...
/*
  For meanings of each data structure member variable, see the
  FAT specification below, as this program was created in accordance
  with the structure documented in the specification found at the
  provided URL:
...
int main(int argc, char * argv[]) {
    ...
    printf("Welcome to my FAT32 image interpreter! For a list of all commands and how to use them, use the 'help' command.\n");
    ...
    while(1) {
        ...
        }else if (strcmp(tokens->items[0], "help") == 0){
            help();
        ...
        for(i = 0; i < 16; i++){
            ...
            if(strcmp(currEntry.DIR_Name, DIRNAME) == 0){
                if(currEntry.DIR_Attr == 0x10){
                    ...
                    fseek(fp, originalPos, SEEK_SET);
                    return 1; // DIRNAME exists, but it is actually a file
                }
            }
        }
...
void help(void){
    ...
    printf("help:                            Prints out this menu as many times as you need.\n\n");
    printf("info:                            Provides a brief summary of the current FAT image being processed.\n\n");
    printf("cd [DIRNAME]:                    Change current working directory to another valid directory inside the current working directory.\n\n");
    printf("ls:                              Print the contents of the current working directory.\n\n");
    printf("size [FILENAME | DIRNAME]:       Provides the size of a file or directory immediately within the current working directory.\n\n");
    printf("lseek [FILENAME] [OFFSET]:       Will set the file pointer of FILENAME, if opened for reading and/or writing, to exactly OFFSET bytes.\n\n");
    printf("read [FILENAME] [BYTES]:         Will read BYTES amount of content (in bytes, of course) inside FILENAME, and output it to the console.\n\n");
    printf("open [FILENAME] [-r|-w|-rw|-wr]: Will open FILENAME for reading and/or writing. Note that -rw and -wr are the same, and both will be accepted.\n\n");
    printf("lsof:                            Will print out a table of all currently opened files.\n\n");
    printf("close [FILENAME]:                Will close FILENAME if currently opened for reading and/or writing.\n\n");
    printf("mkdir [DIRNAME]:                 Will create a directory in the current working directory named DIRNAME if one does not already exist.\n\n");
    printf("creat [FILENAME]:                Will create a file in the current working directory named FILENAME if one does not already exist.\n\n");
    printf("cp [FILENAME] [DESTINATION]:     Will copy FILENAME to DESTINATION. DESTINATION can be inside of a directory, create a new file, or overwrite an existing file.\n\n");
    printf("rm [FILENAME]:                   Will remove a file named FILENAME inside the current working directory if it exists.\n\n");
    printf("rmdir [DIRNAME] [optional: -r]:  Removes a directory named DIRNAME in the current working directory. You can optionally include the -r flag to recursively delete DIRNAME's contents.\n\n");
    printf("dump [FILENAME | DIRNAME]:       Dump the hex contents of a file or directory.\n\n");
}
...
void mkdir(char* DIRNAME){
    ...
    if(result != -1){
        printf("Error: file or directory with that name already exists\n");
    }else if(strstr(DIRNAME, "/") != NULL){
        printf("Error: Directory must be created in the current working directory\n");
    }else if(strlen(DIRNAME) > 8){
        printf("Error: Directory and file names cannot be greater than 8 characters\n");
    }else{
        unsigned long originalPos = ftell(fp);
        fseek(fp, cwd.byteOffset, SEEK_SET);
        unsigned int currCluster = cwd.cluster;
    ...
    if(find(FILENAME) == 1){
        ...
        for(i = 0; i < 10; i++){
            if(openFiles[i].mode != - 1 && strcmp(openFiles[i].path, cwd.path) == 0  && strcmp(openFiles[i].dirEntry.DIR_Name, FILENAME) == 0){
                openFiles[i].mode = -1;
                for(int j = i; j < 9; j++){
                    // When closing a file, move up all other files so that indexes are properly updated.
                    File temp = openFiles[j];
...
void remove_file(char* FILENAME)
{
    ...
    {
        printf("Error: %s is a directory, use rmdir [optional: -r] command instead\n", FILENAME);
        return;
    }
    else if (find(FILENAME) == 1)
    {
...
void remove_directory(char *DIRNAME, char* FLAGS)
{
    ...
    {
        printf("Error: %s is a file, use rm command instead\n", DIRNAME);
        return;
    }
